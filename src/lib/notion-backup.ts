...
export function customerToNotionProps(row: CustomerRow): Record<string, unknown> {
  return {
    ...
    payment_status: notionText(Array.isArray(row.payment_status) ? (row.payment_status as string[]).join(', ') : row.payment_status),
    payment_date: notionNumber(row.payment_date),
