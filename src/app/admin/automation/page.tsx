...
const ALIMTALK_TEMPLATES: AlimtalkTemplate[] = [
  ...
  },
  {
    ...
    trigger: "배정관리 > 작업완료 버튼 클릭 (수동) — 결제방법이 '현금(세금계산서)'인 경우만 발송", triggerType: 'manual',
  ...
  },
  {
    ...
    variables: ['#{고객명}', '#{구글URL}', '#{청소비용}', '#{입금자고객명}'],
    trigger: "배정관리 > 작업완료 버튼 클릭 (수동) — 결제방법이 '카드(온라인 간편결제)'이거나 '플랫폼'인 경우 발송", triggerType: 'manual',
  ...
  },
  {
    ...
    variables: ['#{고객명}', '#{청소비용}'],
    trigger: "작업완료 후 미결제 매일 반복 — 결제방법이 '현금(세금계산서)'인 경우만 발송", triggerType: 'auto', schedule: '매일 06:00 KST',
  ...
  },
  {
    ...
    trigger: "작업완료 후 미결제 매일 반복 — 결제방법이 '카드(온라인 간편결제)'이거나 '플랫폼'인 경우 발송", triggerType: 'auto', schedule: '매일 06:00 KST',
