...
export default function EditEventPage() {
  ...
  useEffect(() => {
    fetch(`/api/events/by-id/${id}`)
      ...
      .then(d => {
        const e = d.event
        if (!e) { toast.error('이벤트를 찾을 수 없습니다'); router.push('/admin/events'); return }
        setSlug(e.slug ?? '')
        setTitle(e.title ?? '')
        setSubtitle(e.subtitle ?? '')
        setBadgeText(e.badge_text ?? '')
        setBadgeColor(e.badge_color ?? 'brand')
        setStartDate(e.start_date ?? '')
        setEndDate(e.end_date ?? '')
        setStatus(e.status ?? 'active')
        setDescription(e.description ?? '')
        setBenefits(Array.isArray(e.benefits) && e.benefits.length > 0 ? e.benefits : [{ ...EMPTY_BENEFIT }])
        setCtaLabel(e.cta_label ?? '지금 신청하기')
        setCtaType(e.cta_type ?? 'kakao')
        setCtaValue(e.cta_value ?? 'https://pf.kakao.com/_bbkkorea')
        setAccentFrom(e.accent_from ?? '#1e8fc0')
        setAccentTo(e.accent_to ?? '#0f5474')
        setIsFeatured(e.is_featured ?? false)
        setSortOrder(e.sort_order ?? 10)
      })
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-text-primary">이벤트 수정</h1>
        <Link
          href={`/events/${slug}`}
          target="_blank"
      ...
      <div className="space-y-5">
        ...
        <section className="bg-surface rounded-2xl border border-border p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputCls} value={badgeText} onChange={e => setBadgeText(e.target.value)} />
            </div>
            <div>
        ...
        <section className="bg-surface rounded-2xl border border-border p-4 space-y-3">
          ...
          {benefits.map((b, i) => (
            <div key={i} className="flex gap-2 items-start">
              <input className="w-10 px-1 py-2 text-sm border border-border rounded-lg text-center bg-surface focus:outline-none" value={b.icon} onChange={e => updateBenefit(i, 'icon', e.target.value)} />
        ...
        <section className="bg-surface rounded-2xl border border-border p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input className={inputCls} value={ctaValue} onChange={e => setCtaValue(e.target.value)} />
            </div>
          </div>
        </section>
        ...
        <section className="bg-surface rounded-2xl border border-border p-4 space-y-4">
          ...
          <div>
            ...
            <textarea className={inputCls} rows={6} value={description} onChange={e => setDescription(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
