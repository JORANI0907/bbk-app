...
export const AUTO_FILL_FIELDS: Record<string, string> = {
  ...
  'customer.care_scope': '케어 범위',
  'customer.special_notes': '고객 요청사항',
  'customer.admin_notes': '관리자 요청사항',
  'customer.memo': '관리자 메모',
  'customer.payment_method': '결제 방법',
  'customer.account_number': '계좌번호',
  'customer.billing_cycle': '결제 주기',
  'customer.door_password': '도어락 비밀번호',
  'customer.parking_info': '주차 정보',
  'customer.elevator': '엘리베이터',
  'customer.building_access': '출입 (신청 여부)',
  'customer.access_method': '출입 방법',
  'customer.business_hours_start': '영업 시작 시간',
  'customer.business_hours_end': '영업 종료 시간',
  'customer.platform_nickname': '플랫폼 닉네임',
...
export function resolveAutoField(
  ...
  const map: Record<string, string> = {
    ...
    'customer.care_scope': String(customer.care_scope ?? ''),
    'customer.special_notes': String(customer.special_notes ?? ''),
    'customer.admin_notes': String(customer.admin_notes ?? ''),
    // 실 DB 컬럼은 customers.notes — customer.memo는 하위 호환용 별칭
    'customer.memo': String(customer.notes ?? ''),
    'customer.payment_method': String(customer.payment_method ?? ''),
    'customer.account_number': String(customer.account_number ?? ''),
    'customer.billing_cycle': String(customer.billing_cycle ?? ''),
    'customer.door_password': String(customer.door_password ?? ''),
    'customer.parking_info': String(customer.parking_info ?? ''),
    'customer.elevator': String(customer.elevator ?? ''),
    'customer.building_access': String(customer.building_access ?? ''),
    'customer.access_method': String(customer.access_method ?? ''),
    'customer.business_hours_start': String(customer.business_hours_start ?? ''),
    'customer.business_hours_end': String(customer.business_hours_end ?? ''),
    'customer.platform_nickname': String(customer.platform_nickname ?? ''),
