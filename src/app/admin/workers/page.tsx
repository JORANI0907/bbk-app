...
export default function WorkersPage() {
  ...
  const fetchWorkers = useCallback(async () => {
    ...
    try {
      ...
      // '전체' (filterType 비어있음) 상태에서는 말소(TERMINATED) + 일용직(DAILY) 자동 제외.
      // 두 유형은 명시적으로 필터에서 '말소' 또는 '일용직' 선택 시에만 노출.
      if (!filterType) list = list.filter(w => w.employment_type !== 'TERMINATED' && w.employment_type !== 'DAILY')
